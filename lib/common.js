 ...
 * 
 * @param {Object} argv all of the yargs
 ...
 */
const modulePathsContainingProcess = (argv, msg, modules, callback) => {
  const pathscontaining = (array, searchmode) => {
    ...
    if (array.length === 1) {
      console.log(`${msg} all ${mname} modules that contain the following string in their registered paths: "${array}"...`);
    } else {
      console.log(`${msg} all ${mname} modules that contain ${searchmode === 'every' ? 'all' : 'any'} of the following strings in their registered paths: ["${array.join('", "')}"]...`);
    }
 ...
 */
exports.argvProcess = (argv, obj) => {
  if (argv.all) {
    console.log(`${obj.msg} all ${exports.mname} registered modules (${Object.keys(obj.modules).length} total)...`);
    ...
    if (argv.modules && argv.modules.length) {
      if (argv.modules.length === 1) {
        ...
        console.log(`${obj.msg} ${exports.mname} modules with names ["${argv.modules.join('", "')}"]...`)
      }
      argv.modules.forEach(obj.specifiedModulesFunc);
    }

    modulePathsContainingProcess(argv, obj.msg, obj.modules, obj.func);
  }
}
