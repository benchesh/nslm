...
exports.run = () => {
  ...
  const checkModule = (moduleName) => {
    ...
    if (fs.existsSync(copiedModuleFilepath)) {
      ...
      if (localModuleActualPath === modules[moduleName].path) {
        ...
        console.log(`${orange(`"${cwdBasename}${localModule}"`)} is NOT pointing to ${orange(`"${modules[moduleName].path}"`)} and will need relinking!`);
      }
    }
