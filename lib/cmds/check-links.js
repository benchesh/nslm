...
exports.run = () => {
  ...
  const checkModule = (moduleName) => {
    ...
    if (localModuleActualPath === modules[moduleName]) {
      ...
      console.log(`${orange(`"${cwdBasename}${localModule}"`)} is NOT pointing to ${orange(`"${localModuleActualPath}"`)} and will need relinking!`);
    }
  }
  ...
  if (fixModules.length) {
    if (argv.fix) {
      ...
      warn(`${fixModules.length} of ${Object.keys(modules).length} checked modules need relinking! Fixing...`);
      require('./link.js').run(true);
    } else {
      warn(`${fixModules.length} of ${Object.keys(modules).length} checked modules need relinking! To fix, simply run this command again with the --fix argument.`);
    }
  } else {
    console.log(`The symlinks for all ${Object.keys(modules).length} checked modules are OK!`);
  }
};
