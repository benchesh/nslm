const { orange } = require('../common.js');
 ...
 */
exports.run = (args) => {
  const argv = yargs(args).array(['modules', 'pathscontaining'])
    .boolean(['all', 'allowmissing'])
    .alias('pathcontaining', 'pathscontaining')
    .argv;
  ...
  if (argv.all) {
    ...
    if (argv.pathscontaining && argv.pathscontaining.length) {
      let pathscontainingFound = false;
      Object.keys(modules).forEach((moduleName) => {
        const modulePath = modules[moduleName];
        if (argv.pathscontaining.every(el => modulePath.includes(el))) {
          putModule(moduleName);
          pathscontainingFound = true;
        }
      });
      if (!pathscontainingFound) {
        console.log(orange(`No modules were found containing the ${pluralise('path', argv.pathscontaining)}:`), argv.pathscontaining);
      }
    }
  }
