...
exports.run = () => {
  ...
  const cwdBasename = `${path.basename(process.cwd())}/node_modules/`;
  ...
  const delinkModule = (moduleName) => {
    ...
    if (!fs.existsSync(pkgPath) || fs.lstatSync(pkgPath).isSymbolicLink()) {
      ...
      if (fs.existsSync(moduleBackupPath)) {
        moveDirIntoDirSync(moduleBackupPath, path.resolve(path.dirname(moduleName)));//move module backup to the local node_modules dir
        console.log(`${green(`"${cwdBasename}${moduleName}"`)} has been delinked & restored from backup`);
      } else if (fs.existsSync(moduleBackupPathOLD)) {
        moveDirIntoDirSync(moduleBackupPathOLD, path.resolve(path.dirname(moduleName)));//move module backup to the local node_modules dir
        console.log(`${green(`"${cwdBasename}${moduleName}"`)} has been delinked & restored from backup`);
      } else {
