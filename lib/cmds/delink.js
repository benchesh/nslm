...
exports.run = () => {
  ...
  const delinkModuleF = (moduleName, subdir = '') => {
    ...
    const pkgPath = path.resolve(moduleName, subdir);
    const moduleBackupPath = path.resolve(moduleBackupsPath, 'node_modules', moduleName, subdir);
    ...
    if (!fs.existsSync(pkgPath) || fs.lstatSync(pkgPath).isSymbolicLink() || fs.existsSync(path.resolve(pkgPath, `.${mname}-copied-dir`))) {
      ...
      if (fs.existsSync(moduleBackupPath)) {
        moveFileOrDirIntoDirSync(moduleBackupPath, path.dirname(path.resolve(moduleName, subdir)));//move module backup to the local node_modules dir
    ...
    if (fs.existsSync(localModuleJsonPath)) {
      ...
      if (subdir) {
        delete localModuleJson[moduleName].subdirs[subdir];
      } else {
  ...
  const delinkModule = (moduleName) => {
    if (localModuleJson[moduleName]) {
      if (localModuleJson[moduleName].subdirs) {
        Object.keys(localModuleJson[moduleName].subdirs).forEach((subdir) => {
          delinkModuleF(moduleName, subdir);
        });
      } else {
        delinkModuleF(moduleName);
      }
    }
  }
