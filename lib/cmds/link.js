...
exports.run = (relink) => {
  ...
  const linkDir = (moduleName, subdir = '') => {
    const jsonObj = jsonFileToJSObj(localModuleJsonPath);
    ...
    const modulePathInJson = jsonObj[moduleName];

    const linktype = (() => {
      if (!relink) return argv.linktype || 'symbolic';
      return fs.existsSync(copiedModuleFilepath) ? 'copy' : 'symbolic';
    })();

    const deep = false;// todo: deep link feature
    ...
    const registeredSubdirPath = path.resolve(modules[moduleName], subdir)

    if (!fs.existsSync(registeredSubdirPath)) {
      error(`"${localModuleBasenamePath}" cannot be linked as the source ${subdir ? 'subdirectory' : 'module'} "${registeredSubdirPath}" doesn't exist!`);
      return;
    }

    if (modulePathInJson && modulePathInJson.linktype !== linktype) {
      if (modulePathInJson.linktype === 'subdirs') {
        if (!subdir) {
          error(`One or more subdirectories have already been linked for "${path.join(cwdBasename, 'node_modules', moduleName)}"! The entire module needs to be delinked first. If you want to delink everything, run: nslm delink -a`);

          return;
        }
      } else {
        error(`"${localModuleBasenamePath}" has already been linked by --linktype=${linktype}! This module needs to be delinked first. If you want to delink everything, run: nslm delink -a`);
    ...
    if (localModuleActualPath === registeredSubdirPath && linktype !== 'copy') {
      ...
      if (linktype === 'copy' && modulePathInJson && fs.lstatSync(localModulePath).isSymbolicLink()) {
      ...
      if (fs.existsSync(localModulePath)) {
        if (!fs.existsSync(copiedModuleFilepath)) {
          ...
          moveFileOrDirIntoDirSync(localModulePath, path.resolve(path.dirname(moduleBackupPath)));
        }
      } else {
      ...
      if (linktype !== 'copy') {
        ...
        fs.symlinkSync(registeredSubdirPath, localModulePath);

        console.log(`${green(`"${localModuleBasenamePath}"`)} has been symlinked to: ${green(`"${registeredSubdirPath}"`)}`);
      }

      const allSubdirs = {
        ...jsonObj[moduleName]?.subdirs,
        ...(subdir && {
          [subdir]: {
            linktype,
            deep,
      ...
      jsonObj[moduleName] = {
        path: registeredModulePath,
        linktype: Object.keys(allSubdirs).length ? 'subdirs' : linktype,
        subdirs: Object.keys(allSubdirs).length ? allSubdirs : undefined,
        deep: subdir ? undefined : deep,
      }

      writeFileSyncRecursive(localModuleJsonPath, JSON.stringify(
        sortObject(jsonObj), null, 4
      ));
    ...
    console.log(`${green(`"${registeredSubdirPath}"`)} is being copied to: ${green(`"${localModuleBasenamePath}"`)}...`);
    ...
    const reportChange = (event) => {
      ...
      tout = setTimeout(() => {
        ...
        let str = `[${('0' + date.getHours()).slice(-2)}:${('0' + date.getMinutes()).slice(-2)}:${('0' + date.getSeconds()).slice(-2)}] Changes detected at "${registeredSubdirPath}":\n`;
    ...
    const chokidarInstance = chokidar.watch('.', {
      ...
      if (!jsonObj[moduleName]) {
        error(waiting ? `"${registeredSubdirPath}" is no longer being watched for changes as "${localModuleBasenamePath}" has been delinked` : `Copy operation has been cancelled as "${localModuleBasenamePath}" has been delinked`);
        chokidarInstance.close();
        return;
      }

      const destPath = path.resolve(localModuleActualPath, filepath);
      const srcPath = path.resolve(registeredSubdirPath, filepath);
      ...
      console.log(`${green(`"${registeredSubdirPath}"`)} has been copied to ${green(`"${localModuleBasenamePath}"`)}${watch ? '. Watching for changes...' : ''}`);
  ...
  if (linkModuleCount === 1) {
    console.log(`${linkModuleCount} ${argv.subdirs ? 'subdir' : 'module'} was linked successfully!`);
  } else if (linkModuleCount) {
    console.log(`${linkModuleCount} ${argv.subdirs ? 'subdirs' : 'modules'} were linked successfully!`);
  } else {
